...
function intCode(input) {
    ...
    for (var i = 0; i < input.length;) {
        var curParam = getParam(i, output);

        //need to set length of instruction, and calculate hte six digit opcode
        var instructionLength;

        if (curParam[curParam.length-1] === "1") {
            output[output[i + 3]] = output[output[i + 1]] + output[output[i + 2]];
            instructionLength = 0;
        }
        if (curParam[curParam.length-1] === "2") {
            output[output[i + 3]] = output[output[i + 1]] * output[output[i + 2]];
        }
        if (curParam[curParam.length-1] === "3") { //(USER) Input
            output = output.replaceAt(output[i+1], codeInput);
        }
        if (curParam[curParam.length-1] === "4") { //Output
            console.log(input[i+1]);
        }
        if (curParam[curParam.length-1] === 99) {
