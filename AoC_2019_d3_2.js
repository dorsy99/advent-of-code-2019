...
function runSteps(input) {
    ...
    stepArray.forEach(function(step){
        if (step[0] == "R") {
            var num = parseInt(step.substr(1,step.lenth));
            for (var i = 0; i < num; i++) {
                x++;
                steps++;
...
//console.log(runSteps(input1).toString());
...
function findCoords(in1, in2) {
    var coordStepCounts = [];
    in1.forEach(function(step1){
        in2.forEach(function(step2){
            //console.log(step1[0].toString() + "," + step1[1].toString());
            if ((step1[0].toString() + "," + step1[1].toString()) == (step2[0].toString() + "," + step2[1].toString())) {
                coordStepCounts.push(parseInt(step1[2]) + parseInt(step2[2]));
            }
        })
    })
    return coordStepCounts;
}
...
function sortNumber(a, b) {
  ...
  stepOut.sort(sortNumber);
  
  console.log("ANSWER = " + stepOut[0]);
