...
function calcRoute(input) {
    ...
    workStr.forEach(function(i) {
        if (i[0] == "R") {
            for (var q = 0; q < parseInt(i.substr(1, i.length)); q++) {
                x++;
                steps++;
                outString.push(x + "," + y + "," + steps);
            }
            //x = x + parseInt();
        }
        if (i[0] == "L") {
            //x = x - parseInt(i.substr(1, i.length));
            for (var q = 0; q < parseInt(i.substr(1, i.length)); q++) {
                x = x - 1;
                steps++;
                outString.push(x + "," + y + "," + steps);
        ...
        if (i[0] == "U") {
            ...
            //y = y + parseInt(i.substr(1, i.length));
        }
        if (i[0] == "D") {
            for (var q = 0; q < parseInt(i.substr(1, i.length)); q++) {
                y = y - 1;
...
var out1 = calcRoute(test11);

var out2 = calcRoute(test12);
